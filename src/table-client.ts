...
export class TableClient<TItem = any> {
  ...
  getRecommendations(): Recommendation[] {
    ...
    const recommendations = this.statsCollector.getRecommendations()
    ...
    return this.executeWithRetry(async () => {
      ...
      if (this.statsCollector) {
        ...
        const stats: OperationStats = {
          ...
          timestamp: startTime,
          latencyMs,
          consumedRCU: response.ConsumedCapacity?.CapacityUnits,
        ...
        this.statsCollector.recordOperation(stats)
      }
      ...
      try {
        ...
        if (this.statsCollector) {
          ...
          const stats: OperationStats = {
            ...
            timestamp: startTime,
            latencyMs,
            consumedWCU: response.ConsumedCapacity?.CapacityUnits,
            itemCount: 1,
          }
          this.statsCollector.recordOperation(stats)
        }
        ...
        while (keysToProcess.length > 0 && attempts < maxAttempts) {
          ...
          if (this.statsCollector) {
            ...
            const stats: OperationStats = {
              ...
              consumedRCU: consumedCapacity?.CapacityUnits,
        ...
        while (operationsToProcess.length > 0 && attempts < maxAttempts) {
          ...
          if (this.statsCollector) {
            ...
            const stats: OperationStats = {
              ...
              timestamp: startTime,
              latencyMs: Date.now() - startTime,
              consumedWCU: consumedCapacity?.CapacityUnits,
  ...
  >(
    ...
    if (this.statsCollector) {
      ...
      const stats: OperationStats = {
        ...
        accessPattern: patternName as string,
        timestamp: startTime,
        latencyMs,
        consumedRCU: result.consumedCapacity?.CapacityUnits,
